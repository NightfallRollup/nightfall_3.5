...
/*
  ...
  - post('/proposer/offchain-transaction') : Processes offchain transaction. This is the endpoint that optimist
  can advertise when registering as a proposer so that clients send transactions here.
  - post('/workers/check-transaction') : Performs several checks on transactions (check duplicate commitment and nullifier, checks
...
async function initWorkers() {
  if (cluster.isPrimary) {
    ...
    app.post('/workers/transaction-submitted', async (req, res) => {
      ...
      try {
        const response = submitTransaction(eventParams);
        res.json(response);
      } catch (err) {
        res.sendStatus(500);
      }
